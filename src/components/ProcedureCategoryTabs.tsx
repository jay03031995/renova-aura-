...
export default function ProcedureCategoryTabs({
  ...
  // A card linking here (e.g. from a doctor's "Treatments performed" list)
  // can pass ?category=Reconstructive%20Surgery so the visitor lands on
  // the tab that actually contains that treatment, instead of always
  // defaulting to the first tab.
  const requestedCategory = useSearchParams().get("category");
  const initialCategory =
    requestedCategory && categories.includes(requestedCategory as ProcedureCategory)
      ? (requestedCategory as ProcedureCategory)
      : categories[0] ?? "";
